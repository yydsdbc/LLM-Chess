...
(function (root) {
  ...
  function create(opts) {
    ...
    var style = opts.style || 'balanced';   // 棋风: aggressive 攻击型 / defensive 防守型 / balanced 均衡型
    ...
    var STYLE_CN = {
      aggressive: '攻击型 — 主动制造威胁与杀势: 兑子开线/车炮深入/弃子抢攻, 但不吃无把握之子',
      defensive: '防守型 — 先保王城与阵型: 补士象/车护肋道/不轻弃兵, 稳健兑子削弱对方攻势',
      balanced: '均衡型 — 两翼均衡出子, 机会好果断进攻, 风险高立即回防'
    };
    ...
    // ── 固定 System Prompt (v1.3 每手相同; v1.5.7: 方向感/分边示例/候选坐标制/防拉锯) ──
    function systemPrompt() {
      var exMove = side === 'red' ? '"from":"h3","to":"e3"' : '"from":"h8","to":"e8"';
      var exCand = side === 'red' ? '"move":"h3-e3","score":"+0.3"' : '"move":"h8-e8","score":"+0.3"';
      // v2.6: 开局三节全程保留 — system 全局恒定是前缀缓存命中前提 (动态裁剪每局制造一次全前缀失效); 节内自带"中残局忽略"自限标记
      return '你是中国象棋AI决策模块, 现在执' + sideCN + '。每轮只输出一行 JSON 原文 (回复的第一个字符必须是 {), 100% 简体中文; 凭直觉一步选定, 禁止深度推演与多步搜索, 不输出思考过程。'
        ...
        + '\n## 评分驱动: 领先2分以上兑子简化; 落后2分以上制造复杂对攻; 均势按棋风; 必杀/白吃大子第一直觉即最终答案。'
        ...
        + '\n## 开局不要镜像: 对方开局不影响独立判断。'
        + '\n## 你的棋风: ' + (STYLE_CN[style] || STYLE_CN.balanced)
