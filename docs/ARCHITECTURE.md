...
flowchart LR
    ...
    subgraph Core [core/ + evaluation/ (browser + node)]
        ...
        AGENT[ai/llm_agent.js + random_agent.js]
    end
    UI --> AGENT --> SRV
    AGENT --> ENG
    AGENT --> EVAL
...
- **ai/** — agents. `llm_agent.js` builds prompts, calls the relay, parses/retries/falls back; `random_agent.js` for zero-config play.
