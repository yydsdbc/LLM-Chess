...
## 同方多 LLM (v1.0.daily)
同一方可以配置多个 LLM 协同推理：模型框逗号分隔多个模型（`glm-a, glm-b`，或跨厂商 `deepseek:deepseek-chat, qwen:qwen3-max`），再选择模式 — **轮换**（每手换下一个模型，各自独立前缀缓存）或 **会诊**（全体并行作答，按落点投票决胜，平票比信心和；投票清单进决策卡，个别选民失败不影响出招，全灭才走随机兑底）。
