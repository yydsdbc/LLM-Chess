...
└── tools/              # npm script entry points (Node-side, zero-dep)
    ├── start.js        #   npm start — background start on port 8788
    ├── stop.js         #   npm stop  — stop the background instance
    └── check.js        #   npm run check — syntax sweep + prompt hard gate
```
...
| `node test/i18n_check.js` | i18n guards, 15 groups (zh/en key parity, placeholder parity, data-i18n / t() coverage, static & JS-side CJK attribute hooks, dynamic write-entry CJK, key-existence for non-first-argument key references, button-label hooks — `data-i18n-title` alone no longer exempts a button's text, preference-option hooks, JS-template `data-i18n*` key existence — the fourth blind spot after index.html and `t()` calls, glyph-only button names — a symbol-only button's accessible name is its own glyph, so `aria-label` is mandatory) |
| `node test/link_check.js` | docs link guard — relative links in all `.md` files must resolve to real files, and links inside `docs/` / `.github/` must start with `../` (a same-directory link resolves fine yet points at the wrong GitHub URL) |
| `node test/check_ui.js` | syntax (17 files) + ID cross-check + script-src existence + localStorage prefix guard + release files + HTML hygiene + PWA manifest (icons/screenshots/categories) + SW guard + replay-dialog semantics + life-cycle generation guards + a11y/PWA source guards (#sr-alert announcer, `#sr-cursor` keyboard-cursor announcement, label `for=`, move-log keyboard, drag abort, end-card focus return, settings-modal shortcut gate, SW full-shell precache + navigation fallback + cache-write catch, piece-glyph single outlet, fallback detection off raw meta) + wire-up/write-point guards (static ids must be taken from `document`, end-card export binding, `dataset.flip` write, hidden `#btn-row` focusability, status-clock ticker, Enter/Space yield to controls, cursor-announcer clearing, replay first paint, panel-stat single outlet, replay-mode AI gate, end-game alias declaration order) + a display-value/timing section (replay toolbar labels from the dictionary, play/pause first paint, dialog semantics + one shared keyboard exit for both full-screen overlays, the repetition-counter key taken before `undoMove`, engine hot-path memos invalidated by `bumpVer`) + an a11y/i18n/hot-path/PWA section (accessible names on the replay toolbar's 17 symbol-only buttons, the record picker / jump box / think-panel pager, `aria-pressed` on the flip + fullscreen toggles, a live region for the provider test-connection result, the replay layer's Tab trap yielding to a higher modal, focus returned to the same move after the list is rebuilt, `th[scope]` on the committee + help tables, move-effect glyphs from the dictionary, the drag ghost's cached size, the splitter/volume persisting only on release, the replay range input coalesced per frame, sw.js's scope-relative API check + cache write on `waitUntil`, and the manifest splash colour against the page's **effective** body rule) + an event-double-fire / ARIA-role / rebuild-focus section (the panel splitter yielding to the already-consumed arrow key and `selfActing` recognising `role="separator"`, the last-move badge as a real button with a live click binding and a `visibility` contract, `role="img"` on board squares and `role="button"` on move-log entries, the end-game card's dismiss flag and its reset, the resume banner's `role="status"`, focus and expanded-state survival across the think-panel and replay-panel rebuilds, `a[href]` in all four focus traps) + a two-entry-points / default-focus / banner-semantics section (the end-game card's backdrop click must share the `dismissEndOverlay` flag with Esc, `role="button"` + `aria-current` on the replay move list, ACAO on the Anthropic upstream-failure 502, Esc passing through the input early-return plus the settings layer declaring it consumed, the `err`-mode banner's announcement / focusability / close binding, the first-paint `data-i18n` status text and pre-`fetch` render, replay board cells' `role="img"` + coordinate labels + `aria-hidden` rulers, the longest-think link's accessible name, and focus hand-back for the resume banner and banner auto-hide) + a sensitive-path / modal-containment / focus-and-state-sync section (the static handler's `config`/`.git`/`logs`/dot-directory denylist and null-byte path guard, ACAO on early rejections and on `health`/`providers`, `sw.js` caching only `status === 200`, the replay layer consuming every unhandled key while open, `role="group"` + `aria-labelledby` on the settings columns, the move-log focus guard for undo/restart, focus retention when the replay transport buttons get disabled, `aria-current` on the main move log, the replay fullscreen button's state- and language-dependent name, `role="status"` on `#server-warn`, the `snapshot()` memo plus removal of the per-move dead `logText` allocation, and a normalisation/routing/relay-outlet/focus-retention/language-repaint section — the denylist evaluated on the normalised path, routing on pathname only, the relay's premature-close outlets and body cap, the service worker's own-prefix cache cleanup, the engine's `gaveCheck` (checkmate counts as a check) and its stats snapshot stack, focus retention when `#btn-row` hides / the replay restore bar hides / the archive buttons get disabled / the replay opener is gone, the banner's role tracking its focusability, the high-speed replay move list keeping `aria-current` in sync, the provider test result using `tArgs` so `{ms}` is really substituted, and the hot-swap repaint of `#server-warn` and the end-game stats) |
| `node test/_replay_edge.js` | replay edge cases + bookmark pure logic (empty/single/two-move records, out-of-range seeks) |
| `node test/_logic_layer.js` | pure logic layer guards (bookmark navigation, record quota/import, Elo math, controller boundaries) + engine `legalTargets`/`dangerTargets`/`inCheck`/`posKey` memo + state-version invalidation + DOM-stub renderer hot-path guards (one `snapshot()` per render, arrow-SVG and sparkline signature dedupe, move-log `tabindex`, banner → `#sr-alert`) + a DOM-stub i18n/glyph block + a `vm`-stubbed service-worker offline-shell guard (install-time shell precache, scope-relative navigation fallback, `waitUntil` cache write) + **L14/L15/L16/L17/L18/L19/L20** (decision-card a11y label against the real dictionary, the repetition counter's undo key timing, sw.js scope-relative `/api/` check, the end-game card's dismiss flag + board-square/move-entry ARIA roles, the banner's `err`-mode announcement plus focus hand-back when it hides, the `snapshot()` memo: same object within a state version, fresh after a move, restored board after undo, and the L20 set: incremental move stats compared against a full replay after **every** move with the sample forced to cover checkmate — the check flag must include `checkmate`, not only `result === 'check'` — plus undo restoring those stats exactly through the snapshot stack with the invariant intact after playing on, and the service worker's `activate` deleting only its own `xq-shell-` prefix so a sibling app on the same origin keeps its cache) |
| `node test/_prompt_level_smoke.js` | prompt-tier injection: each level constant + legacy style map + prefix-cache invariance |
| `node test/replay_risk_check.js` | replay risk detector (incl. the lazy-fill path: jumping to a ply must yield the same risk table and the same 将/杀/困 marks as stepping there) + record quota fallback |
| `node test/_committee_agent.js` | same-side multi-LLM committee: council vote, tie-break, rotation, all-fail, usage sum |
| `node test/_server_http.js` | server.js HTTP behavior, 119 checks (incl. real relay traversal via injected stub upstream) (spawns a real server: health shape+version / static+ETag/304 incl. sw.js and the `?query` form / 404 / path-traversal 403 incl. sibling-prefix dir + backslash form / malformed-encoding 400 / **sensitive-path denylist** (`/config/keys.json` used to be served verbatim with real API keys; `.git/`, `logs/` and dot-directories refused) / **null-byte path guard** (`GET /%00` used to kill the process — liveness re-checked after) / OPTIONS on api+static / bad-json 400 / empty body 400 / >2MB abort / unknown provider 400 / missing-apiKey 400 / missing model-field 400 / **ACAO on every early rejection and on health+providers** / providers shape + no-key-leak + `hasKey` + no-store / HEAD+ETag / manifest+icon+sw MIME / method guards / wrong-ETag 200 / rate-limit 429 + Retry-After + ACAO / OpenAI streaming SSE passthrough / directory 404 / keys.json hot-reload + half-written tolerance / **static-cache invalidation** (changed or same-size-rewritten file must not serve stale bytes) / **upstream request construction** (per-provider custom headers merged without clobbering defaults, `thinking` injected only for GLM-family upstreams and actually injected for them, trailing-slash stripping + default and per-provider `chatPath`, `stream_options` only when streaming, documented defaults, `temperature: 0` surviving the default, `Content-Length` as a **byte** count for CJK prompts) / **upstream non-200 passthrough** (401/429/500 must not be rewritten to 200/502 — a stream error body parsed as SSE ends in the misleading 「流式返回为空」) / **upstream failure → 502** (the error branch used to be untested, so a regression would hang instead of failing fast; both protocol paths must carry ACAO) / **non-object JSON body → 400** (a literal `null` body used to throw inside the async handler and kill the whole process — one POST could remotely kill the relay) / providers `name`/`baseUrl`/`models` passthrough / OPTIONS preflight header **values** / keys.json deleted (ENOENT) tolerance / static `Cache-Control` value / **mid-response upstream abort** (the non-stream paths used to hang forever — the client had to time out after 12s — because Node emits `error` only when a listener exists; both must answer 502 and the process must stay alive) / **non-stream upstream body cap** (16MB, answered with 502 instead of buffering an unbounded body) / **denylist evaluated on the normalised path** (`/x/..%5cconfig/keys.json` used to slip past a raw-first-segment check and return the key file on Windows) / **routing on pathname only** (`POST /api/chat?t=1` used to fall through to the static branch and answer 404) |
| `node test/smoke_relay.js` | real relay single shot (needs a configured key) |
| `node test/smoke_ui.js [model]` | headless Edge UI smoke, 14 checks (auto start / decision log / pager sentinel / screenshot) |
...
`node test/run_tests.js` — 50 assertions green, including the publicly recognized perft numbers:
...
- 4 项总计; test 151 / 88 / 50 / 52 / 10 / 25 / check_ui EXIT 0; 完整清单见 [OPTIMIZATION_LOG.md](OPTIMIZATION_LOG.md) (第 1–11 轮)。
